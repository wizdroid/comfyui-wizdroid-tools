...
_FALLBACK_SYSTEM: Dict[str, str] = {
    "system_prompt_template": (
        "You write Krea 2 character portrait prompts as one or two natural-language "
        "paragraphs. Honor every JSON field; invent coherent detail only where the JSON "
        "is silent. Subject and attributes first, then pose, setting, lighting, camera, "
        "medium. No bullets or headers. If lora_trigger is present, begin with it exactly. "
        "Target about {max_tokens} words. Output only the prompt text.\n\n"
        "## CONTENT RULES\n"
        "{spice_guidance}\n\n{fantasy_guidance}\n\n{detail_guidance}\n\n"
        "Character JSON:\n{character_json}"
    ),
    "user_prompt_template": (
        "Expand this character JSON into a Krea 2 portrait prompt (one or two "
        "natural-language paragraphs). Include every field. Output only the prompt "
        "text:\n{character_json}"
    ),
}
